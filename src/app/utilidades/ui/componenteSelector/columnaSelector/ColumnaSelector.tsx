  ...
  :{planetaSeleccionadoNombre:string, recogerPlaneta:Function,listaPlanetas:Array<any>}){
    ...
    <div className="grid grid-cols-2 gap-4 mt-12">
        {listaPlanetas.map((planeta, indice) => (
            <div 
                key={indice} className={`lg:mr-1.5 lg:ml-1.5 flex flex-col justify-center hover:cursor-pointer hover:scale-110 ${planetaSeleccionadoNombre === planeta.nombre ? " scale-110" : ""}`} 
                onClick={() => {
                    ...
                    <div className="relative text-center">
                        ...
                        <h2 className={`text-xl absolute text-white inset-x-0 bottom-0`}>{planeta.nombre}</h2>
                    </div>
            </div>
        ))}
    </div>)
